...
function Book({ book }) {
  const { thumbnail, author, name, price, rating, featured, id } = book;
  ...
  for (let i = 1; i <= rating; i++) {
    stars.push(
      <svg viewBox="0 0 20 20" fill="currentColor" className="star" key={i}>
        <path
          fillRule="evenodd"
  ...
  return (
    <div className="book-card">
      {!isLoading && isError && <Error error="There was an error occured!" />}
      <img
